...
for t_type, t_num, cmap in zip(tissue_types, tissue_numbers, cmaps):
    ...
    axrows[0][1].imshow(np.rot90(tissue_mask), cmap=cmap, alpha=0.3)
...
for t_type, t_mat, cmap in zip(tissue_types, [gm, wm, csf], cmaps):
    ...
    axrows[1][0].imshow(np.rot90(tissue_mask, 1), cmap=use_cmap, alpha=0.4)
...
for t_num, cmap in zip(tissue_numbers, cmaps):
    ...
    axrows[1][1].imshow(np.rot90(tissue_mask), cmap=cmap, alpha=0.3)
